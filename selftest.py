...
def check_environment() -> tuple[str, int] | tuple[None, int]:
    ...
    scanner_ver = subprocess.run([php, scanner, "--version"],
                                 capture_output=True, text=True).stdout.strip()
    sys.path.insert(0, HERE)
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location(
            "cls", os.path.join(HERE, "classify-identifiers.py"))
        cls = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(cls)
        classifier_ver = cls.TOOL_VERSION
    except Exception:
        classifier_ver = "unknown"

    if not scanner_ver.endswith(classifier_ver):
        print(f"version mismatch: {scanner_ver!r} vs "
              f"classify-identifiers.py {classifier_ver}", file=sys.stderr)
        print("One of the two files is a stale copy. Replace both from the "
              "same source.", file=sys.stderr)
        return None, 1
    ...
    mb = subprocess.run([php, "-r", "echo function_exists('mb_check_encoding') ? 'yes' : 'no';"],
                        capture_output=True, text=True).stdout.strip()
    print(f"tools {classifier_ver} | php {ver} at {php} (ext/mbstring: {mb})")
    return php, 0

...
this repository has **not** been submitted: the concept is under discussion on
`internals@lists.php.net`, and whether it becomes an RFC at all depends on
that discussion. Read `rfc-strict-identifiers.txt` as one proposal the data
would support, not as a decided direction.
...
Two independent corpora were scanned.

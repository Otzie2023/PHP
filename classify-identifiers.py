...
# Must match TOOL_VERSION in scan-identifiers.php. It describes the JSONL
# output contract, not the code, so it only changes when that format changes.
TOOL_VERSION = "1.2.0"

# 1.2.0 adds a "name" record type; 1.1.0 output remains readable, so an
# existing scan does not have to be repeated to be re-classified.
COMPATIBLE_TOOL_VERSIONS = {"1.1.0", "1.2.0"}

# Bumped for classifier changes that do not affect the JSONL format, so a
# re-classification does not require a re-scan.
CLASSIFIER_REVISION = "1.3.0"
...
# Three tiers, because they are three different kinds of evidence.
#
# FIXTURE: input data for parsers, linters and static analysers, whose whole
# purpose is to be pathological. The PHP_CodeSniffer fixtures alone carry emoji
# class names and deliberately case-varied non-ASCII methods. These are not
# evidence of what anybody writes to get work done, and treating them as such
# overstates both the breakage and the benefit of a strict rule.
#
# TEST CODE: ordinary test suites. This is real code somebody wrote on purpose;
# a Japanese PHPUnit method name belongs here, not in FIXTURE.
#
# PRODUCTION: everything else.
FIXTURE_MARKERS = ("/fixtures/", "/Fixtures/", "/fixture/", "/_fixtures/",
                   "/stubs/", "/Stubs/", "/testdata/", "/test-data/",
                   "/_data/", "/expected/", "/snapshots/", "/corpus/")

TEST_MARKERS = ("/tests/", "/test/", "/Tests/", "/Test/", "/spec/", "/Spec/",
                "/testsuite/", "/TestSuite/", "/benchmarks/")


def tier_of(path: str) -> str:
    p = "/" + path
    under_test = any(m in p for m in TEST_MARKERS)
    if any(m in p for m in FIXTURE_MARKERS) or path.endswith(".phpt"):
        return "fixture"
    # PHP_CodeSniffer and friends keep fixture sources as .inc directly under
    # tests/, not in a directory named fixtures. A .inc file under a test path
    # is input data, not a test that runs.
    if under_test and path.endswith((".inc", ".phtml")):
        return "fixture"
    base = path.rsplit("/", 1)[-1]
    if under_test or base.endswith(("Test.php", "TestCase.php")):
        return "test_code"
    return "production"
...
def main() -> int:
    ...
    ap.add_argument("--exclude-package", action="append", default=[], metavar="PKG",
                    help="omit a package from the headline figures and report it "
                         "separately; repeatable. Use when a single package "
                         "dominates the corpus, so that concentration is not "
                         "reported as breadth.")
    args = ap.parse_args()
    ...
    with open(args.jsonl, "r", encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            if rec["type"] == "meta":
                meta = rec
                if rec.get("tool_version") not in COMPATIBLE_TOOL_VERSIONS:
                    sys.exit(
                        f"version mismatch: {args.jsonl} was produced by "
                        f"scan-identifiers.php {rec.get('tool_version')}, but this "
                        f"is classify-identifiers.py {TOOL_VERSION}. One of the two "
                        f"is a stale copy; re-run the scan with matching tools."
                    )
                continue
            if rec["type"] == "name":
                names.append(rec)
    ...
    excluded_names = set(args.exclude_package)
    excluded_rows = [r for r in rows if r["pkg"] in excluded_names]
    if excluded_names:
        rows = [r for r in rows if r["pkg"] not in excluded_names]
        # These three were accumulated while reading, i.e. before the exclusion
        # was applied. Recompute them from the surviving rows, or the summary
        # would report corpus-wide totals next to a filtered record count.
        occurrences = sum(r["occurrences"] for r in rows)
        pkg_with_ident = {r["pkg"] for r in rows}
        file_with_ident = {(r["pkg"], r["file"]) for r in rows}

    if excluded_names:
        names = [r for r in names if r["pkg"] not in excluded_names]

    name_rows = []
    for rec in names:
        info = classify(rec["name_hex"])
        name_rows.append({
            "pkg": rec["pkg"], "file": rec["file"], "line": rec["line"],
            "context": rec["context"], "occurrences": rec["n"],
            "name": info["name"] if info["name"] is not None else "<invalid utf-8>",
            "name_hex": rec["name_hex"], "tier": tier_of(rec["file"]),
            "valid_utf8": info["valid_utf8"], "uax31_profile": info["uax31_profile"],
            "nfc": info["nfc"], "invisible": info["invisible"],
            "scripts": info["scripts"], "mixed_script": info["mixed_script"],
        })
    ...
    summary = {
        "environment": {
            ...
            "php": (meta or {}).get("php_version"),
            "scanner_had_mbstring": (meta or {}).get("mbstring"),
        ...
        "by_tier": dict(collections.Counter(r["tier"] for r in rows).most_common()),
        "production_only": {
            "records": count(lambda r: r["tier"] == "production"),
            "packages_affected": len({r["pkg"] for r in rows if r["tier"] == "production"}),
            "invalid_utf8": count(lambda r: r["tier"] == "production" and not r["valid_utf8"]),
            "not_uax31_profile": count(
                lambda r: r["tier"] == "production" and r["valid_utf8"] and not r["uax31_profile"]),
            "not_nfc": count(
                lambda r: r["tier"] == "production" and r["valid_utf8"] and not r["nfc"]),
            "confusable_script_mix": count(
                lambda r: r["tier"] == "production" and r["mixed_script"]),
            "invisible_characters": count(lambda r: r["tier"] == "production" and r["invisible"]),
        },
        "production_and_test_code": {
            "records": count(lambda r: r["tier"] != "fixture"),
            "packages_affected": len({r["pkg"] for r in rows if r["tier"] != "fixture"}),
            "not_uax31_profile": count(
                lambda r: r["tier"] != "fixture" and r["valid_utf8"] and not r["uax31_profile"]),
            "confusable_script_mix": count(
                lambda r: r["tier"] != "fixture" and r["mixed_script"]),
        },
        "top_packages": dict(collections.Counter(r["pkg"] for r in rows).most_common(15)),
        "top_packages_excluding_fixtures": dict(collections.Counter(
            r["pkg"] for r in rows if r["tier"] != "fixture").most_common(15)),
        ...
        "excluded_packages": {
            ...
            "records": len(excluded_rows),
            "not_uax31_profile": sum(
                1 for r in excluded_rows if r["valid_utf8"] and not r["uax31_profile"]),
            "by_tier": dict(collections.Counter(r["tier"] for r in excluded_rows).most_common()),
        } if excluded_names else None,
        "names_not_identifiers": {
            "note": ("string literals in name-creating or name-looking-up "
                     "positions. These are names, not identifiers: a "
                     "compile-time identifier rule does not govern them. Only "
                     "the statically visible subset is measurable at all."),
            "records": len(name_rows),
            "distinct": len({r["name_hex"] for r in name_rows}),
            "packages_affected": len({r["pkg"] for r in name_rows}),
            "by_context": dict(collections.Counter(
                r["context"] for r in name_rows).most_common()),
            "by_tier": dict(collections.Counter(
                r["tier"] for r in name_rows).most_common()),
            "would_fail_identifier_rule": sum(
                1 for r in name_rows
                if not r["valid_utf8"] or not r["uax31_profile"] or not r["nfc"]),
            "top_packages": dict(collections.Counter(
                r["pkg"] for r in name_rows).most_common(10)),
        },
    ...
    if args.json_path:
        with open(args.json_path, "w", encoding="utf-8") as fh:
            json.dump({"summary": summary, "rows": rows, "names": name_rows}, fh,
